...
- Calcolare automaticamente una stima di costo (min–max) in base alle risposte.
...
- `Visitatore` – utente non autenticato che interagisce con il chatbot
- `Utente` – specializzazione del Visitatore: può fare tutto ciò che fa un Visitatore, più richiedere il sopralluogo dopo aver fornito i propri contatti
- `Amministratore` – rappresenta l'azienda; accede all'area riservata per gestire i lead

> Nota: `Utente` è una **generalizzazione** di `Visitatore` in senso UML: eredita tutte le azioni del Visitatore e ne aggiunge di proprie. Non va confuso con l'autenticazione.
...
4. Inserimento di nome, telefono ed email per ricevere la stima.
5. Calcolo automatico della stima di costo (range min–max) basata sulle risposte.
6. Visualizzazione del riepilogo e della stima con pulsante per richiedere sopralluogo.
...
- Come **amministratore**, voglio vedere tutti i lead ricevuti con i loro dati per poter fare follow-up commerciale.
...
- **Seleziona categoria lavoro**: il bot presenta le categorie disponibili; il visitatore ne sceglie una.
- **Rispondi alle domande guidate**: per la categoria selezionata il bot pone una serie di domande; l'utente risponde una alla volta; le risposte vengono salvate nella sessione.
- **Inserisci dati di contatto**: il bot chiede nome, telefono ed email; i dati vengono validati prima di procedere.
- **Visualizza stima e riepilogo**: il backend calcola la stima min–max; il bot mostra il riepilogo completo con la stima.
...
I casi d'uso non devono essere confusi con i rapporti tra attori. Nel nostro progetto, `Utente` è un attore specializzato di `Visitatore`: può fare tutto ciò che fa un Visitatore, più alcune azioni aggiuntive (richiedere sopralluogo). Questo si modella con una **generalizzazione tra attori**, non con `include` o `extend`.
...
- `Rispondi alle domande guidate` <<include>> `Salva risposta in sessione` – ogni risposta viene sempre registrata.
...
- `Esporta lead in CSV` <<extend>> `Gestisci lead` – l'esportazione è una funzione aggiuntiva, non sempre usata dall'admin.

Aggiungendo queste relazioni, il diagramma diventa più preciso e mette in evidenza i comportamenti obbligatori riutilizzabili e le estensioni opzionali.
...
graph TD
    ...
    subgraph Sistema SmartPreventivo
        ...
        %% Casi d'uso inclusi obbligatori
    ...
    %% Relazioni include (tratteggiato con etichetta)
    UC4 -.->|"<<include>>"| INC1
    UC8 -.->|"<<include>>"| INC2
    UC3 -.->|"<<include>>"| INC3

    %% Relazioni extend (tratteggiato con etichetta)
    UC6 -.->|"<<extend>>"| UC5
    UC9 -.->|"<<extend>>"| UC8
```
...
## 7. Diagramma di flusso (User Flow)

Il diagramma mostra il percorso completo dell'utente dall'apertura del sito fino alla generazione del lead.
...
graph TD
    ...
    E --> |Fotovoltaico| F1[Domande: kW, tipo tetto, utenza]
    E --> |Caldaie| F2[Domande: mq, combustibile, marca]
    E --> |Climatizzatori| F3[Domande: mq, split, BTU]
    E --> |Ristrutturazioni| F4[Domande: mq, tipo lavori]
    E --> |Infissi| F5[Domande: n. finestre, materiale]
    E --> |Impianti Elettrici| F6[Domande: mq, tipo impianto]
...
## 10. Entità e relazioni (schema ER)
...
> Lo schema ER aiuta a progettare le tabelle prima di scrivere il codice. Può essere integrato con un disegno a mano o con uno strumento come dbdiagram.io.

---

## 11. Diagramma UML delle classi
...
classDiagram
    ...
    CategoriaLavoro "1" -- "*" Domanda : contiene
    CategoriaLavoro "1" -- "*" Lead : genera
    SessioneChat "1" -- "*" MessaggioChat : include
    SessioneChat "1" -- "1" Lead : produce
    SessioneChat "1" -- "*" RispostaDomanda : raccoglie
    Domanda "1" -- "*" RispostaDomanda : risposta_a
```

> Il diagramma mostra le classi principali del dominio. I repository (es. `LeadRepository`, `SessioneRepository`) vengono aggiunti separatamente se si vuole rappresentare anche l'architettura dell'applicazione.

---


## 13. Stima costi e tempi di sviluppo (MVP)

| Fase | Attività principali | Giorni stimati | Costo indicativo |
|---|---|---|---|
| **1. Analisi e UI/UX Design** | Requisiti, wireframe chatbot, schema DB | 3 gg | 600 – 900 € |
| **2. Sviluppo Frontend** | Widget chatbot HTML/CSS/JS, responsive mobile-first, dashboard admin | 5 gg | 1.000 – 1.500 € |
| **3. Sviluppo Backend** | Flask + Blueprint, Repository Pattern, motore calcolo, API REST, SMTP | 8 gg | 1.600 – 2.400 € |
| **4. Infrastruttura e DB** | SQLite (sviluppo) / PostgreSQL (produzione), script migrazioni, deploy HTTPS | 3 gg | 600 – 900 € |
| **5. Testing e Rilascio** | Test funzionali, bug fixing, documentazione, deploy finale | 3 gg | 600 – 900 € |
| **TOTALE MVP** | | **22 giorni** | **4.400 – 6.600 €** |

> Nota: il costo è calcolato su una tariffa di 200–300 €/giorno (sviluppatore junior/mid). Per un progetto scolastico il costo reale è 0, ma la stima è necessaria per la materia **GPOI**.

### Stack tecnologico

| Layer | Tecnologia | Motivazione |
|---|---|---|
| **Frontend** | HTML5, CSS3, JavaScript (Vanilla / Bootstrap) | Studiato in classe, leggero, nessuna dipendenza esterna |
| **Backend** | Python 3.x + Flask | Standard 5ª anno; supporta Blueprint e Repository Pattern |
| **Database** | SQLite (sviluppo) / PostgreSQL (produzione) | SQLite per praticità locale, PostgreSQL per deploy reale |
| **Email** | Flask-Mail + SMTP Gmail | Semplice, gratuito per bassi volumi |
| **Autenticazione** | Flask-Login + Werkzeug bcrypt | Standard di sicurezza da classe |
| **Deploy** | PythonAnywhere (gratuito per MVP) | Ottimo per progetti Flask scolastici |
| **Versionamento** | GitHub | Consegna obbligatoria |
